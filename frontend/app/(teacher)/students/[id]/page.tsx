        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {profile.class_name && <span className="bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-400 text-xs px-2.5 py-1 rounded-full font-medium">{profile.class_name}</span>}
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">
              ...
              {classes.map((c) => (
                <button key={c.id} onClick={() => linkClass(c.id)} disabled={classLinking}
                  className={`w-full text-left px-3 py-2 text-sm rounded-lg transition-colors ${profile.class_id === c.id ? "bg-indigo-200 dark:bg-indigo-800 text-indigo-800 dark:text-indigo-200 font-semibold" : "hover:bg-indigo-100 dark:hover:bg-indigo-900/40 text-gray-700 dark:text-gray-300"}`}>
                  {c.name} <span className="text-gray-400 dark:text-gray-500 text-xs ml-1">({c.grade} · {c.subject})</span>
                </button>
              ))}
